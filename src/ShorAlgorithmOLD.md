namespace ShorAlgorithm {
    ...
    operation AppliquerMultiplicationModulaire(a : Int, N : Int, n : Int, qubitControle : Qubit, registreCible : Qubit[], i : Int, tableauC : Int[]) : Unit {
        ...
        // Puis avec le qubit contrôleur yi on applique les 3 portes d'addition modulaire pour ajouter K*yi au registre cible
        ...
        for j in 0..(n-1) {
            ...
            //------------------ Première addition ------------------
            // On ajoute C-N au registre cible (on ajoute la constante C puis on soustrait N pour voir si on déborde ou pas)
            use condition_addition = Qubit();
            // On active le qubit de condition (vaut 1 si qubitControle ET registreCible[j] sont à 1)
            Controlled X([qubitControle, registreCible[j]], condition_addition);
            // On fait l'addition à l'aide de ce qubit de condition
            Controlled AjouterConstanteQFT([condition_addition], (fractionC, registreCible));
            // On désintrique le qubit de condition pour le réutiliser
            Controlled X([qubitControle, registreCible[j]], condition_addition);
            ...
            // On libère les deux qubits auxiliaires utilisés pour la condition et la retenue
            Reset(qubitRetenue);
            Reset(condition_addition);
        }
    }

    operation AppliquerExponentiationModulaire(a : Int, N : Int, n : Int, registreControle : Qubit[], registreCible : Qubit[], tableauC : Int[]) : Unit {
        ...
        for i in 0..Length(registreControle)-1 {
            AppliquerMultiplicationModulaire(a, N, n, registreControle[i], registreCible, i, tableauC);
        }
    ...
    operation CalculerR(a : Int, N : Int, n : Int, tableauC : Int[]) : Int {
        ...
        AppliquerExponentiationModulaire(a, N, n, etat1, etat2, tableauC);
